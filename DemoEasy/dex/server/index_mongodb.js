...
app.post('/swap', async (req, res) => {
    const { buyToken, sellToken, tokenOneAmount, tokenTwoAmount} = req.body;

    try {
        
        // Insert user info
        await new Promise((resolve, reject) => {
            const sql3 = "INSERT INTO transaction (token1_address, token2_address, token_amount1, token_amount2) VALUES (?, ?, ?, ?)";
            db.query(sql3, [buyToken, sellToken, tokenOneAmount, tokenTwoAmount], (err, result) => {
                if (err) {
                    console.error('Error registering user:', err);
                    reject(err);
                } else {
